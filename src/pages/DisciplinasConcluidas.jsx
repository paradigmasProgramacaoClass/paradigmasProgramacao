...
export default function DisciplinasConcluidas() {
  const { concluidas, loading } = useDisciplinas()
                  ...
                  <tbody>
                    {concluidas.map((d) => (
                      <tr key={d.getId()} className="border-b border-transparent hover:bg-regular-100/30 transition-colors">
                        ...
                        <td className="py-5 px-6 font-medium text-regular-500">{d.getProfessor()}</td>
                        <td className="py-5 px-6 font-bold text-primary">{d.getCreditos()}/200</td>
